...
export const cleanupUploadedFiles = (videoFile, thumbnailFile) => {
...
export const uploadVideoService = async (userId, title, description, videoFile, thumbnailFile) => {
    if (!title?.trim())
...
export const updateLikesService = async (videoId, userId) => {
...
export const getVideoByIdService = async (vid) => {
...
export const searchVideosService = async (q) => {
    if (!q)
...
export const downloadVideoService = async (videoId, userId) => {
    ...
    if (downloadsInLast24h >= downloadLimit) {
        ...
        throw new ApiError(400, `Download limit reached. Try again in ${remainingHours} hour(s).`);
    }
...
export const getAllDownloadsService = async (userId) => {
...
export const getDownloadCountService = async (userId) => {
    if (!isValidObjectId(userId))
        throw new ApiError(400, "Not valid User Id");
    const count = await DownloadModel.countDocuments({ userId });
    return count;
};
export const getTodayStatsService = async (userId) => {
    if (!isValidObjectId(userId))
        throw new ApiError(400, "Not valid User Id");
    const ONE_DAY_AGO = new Date(Date.now() - 24 * 60 * 60 * 1000);
    ...
    const watchHistories = await VideoHistory.find({
        ...
        .lean();
    const todayWatchSeconds = watchHistories.reduce((sum, h) => sum + (h.totalWatchedSeconds || 0), 0);
...
export const getAllHistoryService = async (userId) => {
    if (!isValidObjectId(userId))
        throw new ApiError(400, "Not valid User Id");
