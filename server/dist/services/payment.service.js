...
dotenv.config();
export const createOrderService = async (userId, subscriptionPlan, isYearlySubscriptionPlan) => {
...
export const verifyPaymentService = async (userId, razorpay_order_id, razorpay_payment_id, razorpay_signature) => {
    ...
    if (user?.email) {
        sendInvoiceEmail(user.email, user.name || "User", payment.plan, payment.amount, payment.isYearly, razorpay_payment_id, expiry).catch((err) => console.error("Invoice email failed:", err));
    }
    return true;
};
